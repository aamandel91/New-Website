...
const caStreetSpaceDirectional = {
...
module.exports = {
  parseAddress: function (address) {
    ...
    if (addressParts.length === 1) {
      ...
      const spaceDirections = new RegExp(
        '(' + Object.keys(caStreetSpaceDirectionalKeys).join('|') + ')',
        'i'
      )
      if (streetString.match(spaceDirections)) {
        caStreetSpaceDirectionalKeys.forEach((d) => {
          streetString = streetString.replace(
            new RegExp(`(${d})`, 'i'),
            caStreetSpaceDirectional[d]
          )
        })
      }
      ...
      var reStreet = new RegExp(
        '\.\*\\b(?:' +
          Object.keys(streetTypes).join('|') +
          ')\\b\\.?' +
          '( +(?:' +
          streetDirectionalString +
          ')\\b)?',
        'i'
      )
      ...
      var reAveLetter = new RegExp(
        '\.\*\\b(ave.?|avenue)\.\*\\b[a-zA-Z]\\b',
        'i'
      )
      ...
      if (streetString.match(reAveLetter)) {
        ...
        if (result.addressLine1.match(re)) {
          ...
          result.streetDirection =
            streetDirectional[
              f.toUpperCase() +
                (rest.length > 2 ? rest.join('').toLowerCase() : rest.join(''))
            ]
        }
        ...
        if (streetParts.length > 2) {
          ...
          result.streetSuffix = toTitleCase(
            streetTypes[streetParts[streetParts.length - 1].toLowerCase()]
          )
        }
