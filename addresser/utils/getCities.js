...
async function getDataForCountry(country_code) {
  ...
  var states = JSON.parse(
    fs.readFileSync('../data/' + country_code_lower + '-states.json')
  )

  unzipper.Open.url(
    request,
    'https://download.geonames.org/export/zip/' + country_code_upper + '.zip'
  )
    .then(function (d) {
    ...
    .then(function (d) {
      ...
      citiesData.forEach(function (element) {
        ...
        if (
          cityData[4] &&
          cityData[4].length == 2 &&
          cityHash.hasOwnProperty(cityData[4])
        ) {
      ...
      fs.writeFile(
        '../data/' + country_code_lower + '-cities.json',
        JSON.stringify(cityHash),
        'utf8',
        function () {
          console.log(cityHash)
        }
...
;(async () => {
  ...
  mergeFiles(
    ['../data/us-cities.json', '../data/ca-cities.json'],
    '../data/cities.json'
  )
  mergeFiles(
    ['../data/us-states.json', '../data/ca-states.json'],
    '../data/states.json'
  )
})()
