...
export const secureFubAvmLink = (
  clientUrl: string,
  clientId: string | number,
  salt?: string
): string | undefined => {
  const url = clientUrl.replace('[CLIENT_ID]', clientId.toString())
  return salt && 0 < salt.length
    ? url + `?s=${calcSignature(clientId.toString(), salt)}`
    : url
}
...
export function maybeRole(
  ...
  return Object.values(UserRole).find((r) => r === roleNumber) as
    | UserRole
    | undefined
}
