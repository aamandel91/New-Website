...
export class XFFProvider {
  ...
  isEnabled() {
    return (
      this.shouldProxy &&
      container.resolve<AppConfig>('config').repliers.proxy_xff &&
      this.xff.length > 0
    )
  }
  getHeader() {
  ...
  private getClientIp(ctx: Context): string {
    ...
    const lastIp =
      typeof ips[lastIndex] === 'string' ? sanitizeIpAddr(ips[lastIndex]) : ''
    switch (true) {
  ...
  private getClientIpBehindSSR(ips: string[]): string {
    ...
    while (0 < offset) {
      ...
      if (desiredIndex < 0 || typeof ips[desiredIndex] !== 'string') {
        debug(
          `[getClientIpBehindSSR]: offset ${offset} -> ${desiredIndex} is out of bounds for ips: ${ips}. Decreasing offset`
        )
        offset -= 1
        continue
      }
