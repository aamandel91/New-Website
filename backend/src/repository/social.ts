...
export class SocialRepository {
  ...
  async getUserTokens(
    email: string,
    type: SocialProvider
  ): Promise<SocialTable | null>
  async getUserTokens(
    email: string,
    type?: SocialProvider
  ): Promise<SocialTable[] | Pick<SocialTable, 'type' | 'tokens'> | null> {
    const query = this.db
      .table('social')
      .select<'type' | 'tokens'>('type', 'tokens')
      .where({
        email
      })
    if (type) {
  ...
  updateAccessToken(email: string, type: SocialProvider, access_token: string) {
    return this.db
      ...
      .update(
        'tokens',
        this.db.raw(
          `jsonb_set(tokens, '{access_token}', '"${access_token}"', true)`
        )
      )
  ...
  updateRefreshToken(
    ...
    return this.db
      ...
      .update(
        'tokens',
        this.db.raw(
          `jsonb_set(tokens, '{refresh_token}', '"${refresh_token}"', true)`
        )
      )
      .update('updated_at', this.db.fn.now())
      .where({
