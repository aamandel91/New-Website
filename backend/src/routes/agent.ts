...
import {
  agentsCreateClientSchema,
  agentsCreateEstimateSchema,
  agentsGetEstimateSchema,
  agentsSendEstimateSchema,
  agentsCreateMessageSchema,
  agentsGetMessagesSchema,
  agentsUpdateEstimateSchema,
  agentGetBossPeopleSchema,
  agentGetClientsSchema,
  agentUpdateClientSchema,
  agentsDeleteEstimateSchema,
  agentGetSingleClientsSchema
} from '../validate/agent.js'
...
router.param('clientId', async (clientId, ctx: Context, next: Next) => {
  ...
  const [hasValidSignature, borrowedAgentId] =
    await agentService.checkSignature(clientId, ctx.query['s'])
...
router.param('estimateId', async (estimateId, ctx: Context, next: Next) => {
  ...
  const [hasValidSignature, borrowedAgentId] =
    await agentService.checkSignature(
      estimate.clientId.toString(),
      ctx.query['s']
    )
  ctx.assert(hasValidSignature, 403, 'Invalid signature')
  if (borrowedAgentId) {
...
router.post(
  ...
  async (ctx, next) => {
    ctx.state['enable.xff'] = true
    const { error, value } = agentsCreateClientSchema.validate({
      ...ctx.request.body,
      status: true,
  ...
  async (ctx, next) => {
    ...
    const selector = ctx.state.container.resolve(
      SelectAgentClientRegistrationParams
    )
...
router.get('/client', async (ctx) => {
  ctx.state['enable.xff'] = true
  const { error, value } = agentGetClientsSchema.validate({
    ...ctx.query,
    agentId: ctx.state['user'].sub
  })
  if (error) {
...
router.get('/client/:clientId', async (ctx) => {
  ctx.state['enable.xff'] = true
  const { s, ...query } = ctx.query
  const { error, value } = agentGetSingleClientsSchema.validate({
    ...query,
...
router.patch('/client/:clientId', async (ctx) => {
  ctx.state['enable.xff'] = true
  const { error, value } = agentUpdateClientSchema.validate({
    ...ctx.request.body,
    clientId: ctx.params['clientId'],
    agentId: ctx.state['user'].sub
  })
...
router.post(
  ...
  async (ctx, next) => {
    ctx.state['enable.xff'] = true
    const { error, value } = agentsCreateEstimateSchema.validate({
      ...ctx.request.body,
      clientId: ctx.params['clientId'],
      agentId: ctx.state['user'].sub
    })
  ...
  (ctx, next) => {
    ...
    const selectEstimateParams = ctx.state.container.resolve(
      SelectAgentEstimateParams
    )
...
router.get('/estimate/:clientId', async (ctx) => {
  ctx.state['enable.xff'] = true
  const { error, value } = agentsGetEstimateSchema.validate({
    clientId: ctx.params['clientId'],
    agentId: ctx.state['user'].sub
  })
  if (error) {
...
/* ACCESS: YES signature accsess */
router.post(
  '/estimate/:estimateId/send',
  async (ctx, next) => {
    ctx.state['enable.xff'] = true
    const { error, value } = agentsSendEstimateSchema.validate({
  ...
  async (ctx) => {
    try {
      ...
      const baseEventCollectionSelector = ctx.state.container.resolve(
        BaseEventCollectionSelector
      )
      ...
      if (bossPersons?.people?.[0]?.id) {
        const addressShort = estimate.payload.address
          ? baseEventCollectionSelector.addressShort(estimate.payload.address)
          : 'unknown address'
...
router.patch('/estimate/:estimateId', async (ctx) => {
  ctx.state['enable.xff'] = true
  const { error, value } = agentsUpdateEstimateSchema.validate({
    ...ctx.request.body,
    estimateId: ctx.params['estimateId'],
    agentId: ctx.state['user'].sub
  })
...
router.delete('/estimate/:estimateId', async (ctx) => {
  ctx.state['enable.xff'] = true
  const { error, value } = agentsDeleteEstimateSchema.validate({
    estimateId: ctx.params['estimateId'],
    agentId: ctx.state['user'].sub
  })
  if (error) {
...
router.post('/messages/:clientId', async (ctx) => {
  ctx.state['enable.xff'] = true
  const { error, value } = agentsCreateMessageSchema.validate({
    ...ctx.request.body,
    clientId: ctx.params['clientId'],
    agentId: ctx.state['user'].sub
  })
...
router.get('/messages/:clientId', async (ctx) => {
  ctx.state['enable.xff'] = true
  const { error, value } = agentsGetMessagesSchema.validate({
    clientId: ctx.params['clientId'],
    agentId: ctx.state['user'].sub,
...
router.get('/boss/people', async (ctx) => {
  ...
  const { error, value } = agentGetBossPeopleSchema.validate({
    ...ctx.query,
    agentId: ctx.state['user'].sub,
