...
import {
  userAssetsCreateSchema,
  userAssetsGetSchema,
  userAssetsRemoveSchema
} from '../../validate/user/assets.js'
...
router.post('/:type', authMiddleware, async (ctx) => {
  const { error, value } = userAssetsCreateSchema.validate({
    ...ctx.request.body,
...
router.get('/:type', authMiddleware, async (ctx) => {
  const { error, value } = userAssetsGetSchema.validate({
    type: ctx.params['type'],
    email: ctx.state?.['user']?.email
  })
  if (error) {
...
router.post('/:type/remove', authMiddleware, async (ctx) => {
  const { error, value } = userAssetsRemoveSchema.validate({
    ...ctx.request.body,
    type: ctx.params['type'],
    email: ctx.state?.['user']?.email
  })
