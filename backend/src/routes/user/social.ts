...
import {
  userSocialCallbackSchema,
  userSocialGetTokensSchema,
  userSocialRefreshSchema,
  userSocialUnlinkSchema,
  userSocialUrlSchema
} from '../../validate/user/social.js'
...
router.get('/:provider/url', authMiddleware, async (ctx) => {
  const { error, value } = userSocialUrlSchema.validate({
...
router.get('/:provider/cb', authMiddleware, async (ctx) => {
  const { error, value } = userSocialCallbackSchema.validate({
...
router.get('/:provider/refresh', authMiddleware, async (ctx) => {
  const { error, value } = userSocialRefreshSchema.validate({
    provider: ctx.params['provider'],
    email: ctx.state['user'].email
  })
  if (error) {
...
router.get('/:provider*', authMiddleware, async (ctx) => {
  const { error, value } = userSocialGetTokensSchema.validate({
    provider: ctx.params['provider'],
    email: ctx.state['user'].email
  })
  if (error) {
...
router.delete('/:provider', authMiddleware, async (ctx) => {
  const { error, value } = userSocialUnlinkSchema.validate({
    provider: ctx.params['provider'],
    email: ctx.state['user'].email
  })
  if (error) {
