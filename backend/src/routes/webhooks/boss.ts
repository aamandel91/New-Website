...
const bossAuthorizer = container.resolve<Middleware>(
  'middleware.boss.webhook.auth'
)
router.post('/people', bossAuthorizer, async (ctx) => {
  const { error, value } = bossWebhooksEventSchema.validate({
    ...ctx.request.body
  })
  if (error) {
  ...
  const store = ctx.state.container
    .resolve<AsyncLocalStorage<IAsyncLocalStore>>('async_local_store')
    .getStore()
