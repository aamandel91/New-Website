...
export default class AuthService {
  ...
  async signup(params: UserSignupDto): Promise<AuthLoginResponseDto> {
    ...
    if (params.utmSource !== undefined)
      trafficSourceInput.utmSource = params.utmSource
    if (params.utmMedium !== undefined)
      trafficSourceInput.utmMedium = params.utmMedium
    if (params.utmCampaign !== undefined)
      trafficSourceInput.utmCampaign = params.utmCampaign
    if (params.utmTerm !== undefined)
      trafficSourceInput.utmTerm = params.utmTerm
    if (params.utmContent !== undefined)
      trafficSourceInput.utmContent = params.utmContent
    if (params.referer !== undefined)
      trafficSourceInput.referer = params.referer
    if (params.landingPage !== undefined)
      trafficSourceInput.landingPage = params.landingPage
    const trafficSourceData =
      await this.trafficSourceService.storeTrafficSource(trafficSourceInput)

    // Report registration with traffic source information
    const trafficSourceForReport: any = {}
    if (trafficSourceData) {
      if (trafficSourceData.utmSource !== undefined)
        trafficSourceForReport.utmSource = trafficSourceData.utmSource
      if (trafficSourceData.utmMedium !== undefined)
        trafficSourceForReport.utmMedium = trafficSourceData.utmMedium
      if (trafficSourceData.utmCampaign !== undefined)
        trafficSourceForReport.utmCampaign = trafficSourceData.utmCampaign
      if (trafficSourceData.trafficType !== undefined)
        trafficSourceForReport.trafficType = trafficSourceData.trafficType
      if (trafficSourceData.landingPage !== undefined)
        trafficSourceForReport.landingPage = trafficSourceData.landingPage
    }
    this.reportClientRegistration(
      userInfo,
      params.referer || '',
      trafficSourceData ? trafficSourceForReport : undefined
    )
  ...
  async useRepliersToken(params: AuthRepliersTokenDto) {
    ...
    if (
      new Date().getTime() - new Date(message.delivery.sentDateTime).getTime() >
