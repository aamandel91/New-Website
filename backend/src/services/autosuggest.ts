...
export default class AutosuggestService {
  ...
  public async search(params: AutosuggestDto) {
    ...
    const mapboxRequest = this.mapboxService
      .suggest(searchSession(params), {
        q: params.q,
      ...
      .then(({ suggestions }) =>
        suggestions // can be merged with map via reduce but will be less readable
          .filter((v) =>
            locationsFilter(v, this.config.mapbox.autosuggest.max_distance)
          )
          .filter((v) => {
            ...
            for (const prev of jarohash) {
              result =
                result &&
                distance(normalizedName, prev) <
                  this.config.mapbox.autosuggest.jaro_winkler_distance
            }
            jarohash.push(normalizedName)
            return result
          })
          .slice(0, params.resultsPerPage)
          .map(
            ({
              name,
              mapbox_id,
              feature_type,
              context: { region, postcode, place, neighborhood }
            }) => ({
              name,
              mapbox_id,
              feature_type,
    ...
    const listingsRequestParams = {
      ...
      searchFields:
        'address.streetName,mlsNumber,address.zip,address.streetNumber',
    ...
    const listingsRequest = this.repliersService.listings.search(
      listingsRequestParams
    )
    const [mapbox, listings] = await Promise.all([
      mapboxRequest,
      listingsRequest
    ])
    return {
  ...
  private async mapboxAddress(params: AutosuggestAddressDto) {
    ...
    const mapboxRequest = await this.mapboxService.suggest(
    ...
    return mapboxRequest.suggestions
      .filter((v) =>
        locationsFilter(v, this.config.mapbox.autosuggest.max_distance)
      )
      .map(
        ({
          ...
          mapbox_id,
          context: { region, postcode, country, address: ctxAddress, place }
        }) => {
          const { streetName, streetSuffix, streetDirection } =
  ...
  private async googleAddress(params: AutosuggestAddressDto) {
    ...
    const { predictions, status } = await this.googleService.autocomplete({
      input: params.q,
      ...
      .map((prediction) => {
        try {
          ...
          return {
            ...
            streetSuffix: this.mapStreetSuffix(
              parsedAddress.streetSuffix,
          ...
          debug(
            '[googleAddress]: Error parsing address for prediction %o: %s',
            prediction,
            error
          )
          return null
  ...
  private mapStreetSuffix(suffix: string, streetName: string) {
    ...
    if (suffix) {
      ...
      return (
        this.suffixMap.get(maybeSufix || defaultSuffixMapKey) ||
        defaultSuffixMapValue
      )
    }
  }
}
function locationsFilter(v: MapboxSuggestion, distance: number): boolean {
  return (
    (config.mapbox.autosuggest.region_code
      ? v.context.region.region_code === config.mapbox.autosuggest.region_code
      : true) && v.distance <= distance
  )
}
function normalizedLocationName(v: MapboxSuggestion): string {
  const {
    name,
    context: { region, place }
  } = v
  return (
    name.toLowerCase() +
