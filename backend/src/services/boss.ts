...
export type BossWebhookEventName =
  | 'peopleCreated'
  | 'peopleUpdated'
  | 'peopleDeleted'
  | 'peopleTagsCreated'
...
export type BossCollectionResponse<Collection extends BossCollection> =
  ApiResponse & {
    _metadata: {
...
export interface BossWebhooksSingle<
  Event extends BossWebhookEventName = BossWebhookEventName
> {
  id: number
...
export interface BossPeopleGetResponse
  extends BossCollectionResponse<'people'> {
  people: Array<BossPeopleSingle>
}
export interface BossWebhooksCreateRequest<Event extends BossWebhookEventName>
  extends ApiRequest {
  event: Event
  url: string
}
export interface BossWebookCreateResponse<Event extends BossWebhookEventName>
  extends ApiResponse,
    BossWebhooksSingle<Event> {}
export interface BossWebhooksListResponse
  extends BossCollectionResponse<'webhooks'> {
...
export default class BossService {
  ...
  private async request<Response>(
    ...
    if (!this.isEnabled) {
      this.loggerGlobal.warn(
        `[BossService]: Boss API is disabled, skipping request ${method} ${url}`
      )
      return {} as Response
    }
  ...
  public async getPeople(params: BossPeopleGetRequest) {
    ...
    let fields =
      'id,name,firstName,lastName,emails,phones,picture,tags,assignedUserId,addresses'
  ...
  public async webhookCreate<Event extends BossWebhookEventName>(
    params: BossWebhooksCreateRequest<Event>
  ) {
  ...
  public static getPrimaryEmail(emails: BossPeopleSingle['emails']) {
    return emails.find(
      (email) => email.status === 'Valid' && email.isPrimary === 1
    )
  }
  public static getPrimaryPhone(phones: BossPeopleSingle['phones']) {
    return phones.find(
      (phone) => phone.status === 'Valid' && phone.isPrimary === 1
    )
  }
  public static checkTags(person: BossPeopleSingle, config_tags: string) {
    const commonTags = person.tags.filter((value) =>
      config_tags.split(',').includes(value)
    )
