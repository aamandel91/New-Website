...
import {
  BossPeopleWebhookCreateDto,
  BossPeopleWebhookDeleteDto,
  BossPeopleWebhookUpdateDto
} from '../../streams/boss/people.js'
import { WebhooksRepository } from '../../repository/webhooks.js'
import RepliersClients, {
  RplClientsClient
} from '../../services/repliers/clients.js'
...
export default class BossWebhooksService {
  ...
  constructor(
    ...
    private repliersAgents: RepliersAgents
  ) {
  ...
  async installHooks() {
    if (!this.config.boss.webhook.enabled) {
      this.loggerGlobal.info(
        '[BossWebhooksService]: Webhooks are disabled, skipping webhook auto-installation, you can enable explicitly via BOSS_WEBHOOK_ENABLED = true in .env'
      )
      return
    }
    if (this.config.boss.webhook.enabled && !this.config.boss.enabled) {
      this.loggerGlobal.error(
        '[BossWebhooksService]: Webhooks are enabled, but BOSS is disabled, skipping webhook auto-installation, you can enable explicitly via BOSS_ENABLED = true in .env'
      )
      return
    }
  ...
  async removeOldHooks() {
    this.loggerGlobal.info(
      '[BossWebhooksService]: Checking for old/stale webhooks'
    )
    ...
    for (const hook of hooks.webhooks) {
      this.loggerGlobal.info(
        '[BossWebhooksService: removeOldHooks]: Removing webhook ID: %s',
        hook.id
      )
  ...
  async uninstallHooks() {
    ...
    for (const id of this.runningWebhooksIds) {
      this.loggerGlobal.info(
        '[BossWebhooksService: uninstallHooks]: Removing webhook ID: %s',
        id
      )
  ...
  async installPeopleCreateWebhook() {
    ...
    if (webhook && webhook.id) {
      this.loggerGlobal.info(
        ...
        '[BossWebhooksService: installPeopleCreateWebhook]: People created webhook installed: %s',
        webhookUrl
      )
      this.runningWebhooksIds.push(webhook.id)
    } else {
      this.loggerGlobal.warn(
        '[BossWebhooksService: installPeopleCreateWebhook]: Failed to install People created webhook: %O',
        webhook
      )
    }
  }
  async installPeopleUpdateWebook() {
    ...
    if (webhook && webhook.id) {
      this.loggerGlobal.info(
        ...
        '[BossWebhooksService: installPeopleUpdateWebook]: People updated webhook installed: %s',
        webhookUrl
      )
      this.runningWebhooksIds.push(webhook.id)
    } else {
      this.loggerGlobal.warn(
        '[BossWebhooksService: installPeopleUpdateWebook]: Failed to install People updated webhook: %O',
        webhook
      )
    }
  }
  async installPeopleRemoveWebook() {
    ...
    if (webhook && webhook.id) {
      this.loggerGlobal.info(
        ...
        '[BossWebhooksService: installPeopleRemoveWebook]: People deleted webhook installed: %s',
        webhookUrl
      )
      this.runningWebhooksIds.push(webhook.id)
    } else {
      this.loggerGlobal.warn(
        '[BossWebhooksService: installPeopleRemoveWebook]: Failed to install People deleted webhook: %O',
        webhook
      )
    }
  }
  async handlePeopleEvents<EventType extends BossWebhookEventName>(
    body: BossWebhooksEventDto<EventType>
  ) {
    this.logger.info(
      {
        data: {
      ...
      '[BossWebhooksService: handlePeopleEvents:] Handling event type: %s',
      body?.event
    )
    switch (body.event) {
      case 'peopleUpdated':
        return this.handlePeopleUpdated(
          body as BossWebhooksEventDto<'peopleUpdated'>
        )
      case 'peopleCreated':
        return this.handlePeopleCreated(
          body as BossWebhooksEventDto<'peopleCreated'>
        )
      case 'peopleDeleted':
        return this.handlePeopleDeleted(
          body as BossWebhooksEventDto<'peopleDeleted'>
        )
      default:
        this.logger.warn(
          '[BossWebhooksService: handlePeopleEvents:] Unknown event type: %s',
          body.event
        )
        return
    }
  ...
  async handlePeopleUpdated(body: BossWebhooksEventDto<'peopleUpdated'>) {
    ...
    for (
      let offset = 0;
      offset < body.resourceIds.length;
      offset += WEBHOOK_BATCH_SIZE
    ) {
      ...
      for (const event of events) {
        await jsc.publish(
          `${this.config.nats.worker.consumer_stream}.people.update`,
          JSON.stringify({
            id: event.id,
  ...
  async handlePeopleCreated(body: BossWebhooksEventDto<'peopleCreated'>) {
    ...
    for (
      ...
      for (const event of events) {
        jsc.publish(
          `${this.config.nats.worker.consumer_stream}.people.create`,
          JSON.stringify({
            id: event.id,
            payload: event.payload
          }),
  ...
  async handlePeopleDeleted(body: BossWebhooksEventDto<'peopleDeleted'>) {
    ...
    for (const event of events) {
      jsc.publish(
        `${this.config.nats.worker.consumer_stream}.people.delete`,
        JSON.stringify({
          id: event.id,
          payload: event.payload
        }),
  ...
  // CONTEXT: WORKER
  async processPeopleUpsert(
    params: BossPeopleWebhookUpdateDto | BossPeopleWebhookCreateDto
  ) {
    const { id, payload } = params
    debug('[Worker: processPeopleUpsert]: payload: %O', payload)
    try {
      const hasTags = BossService.checkTags(
        payload.person,
        this.config.boss.webhook.client_tags
      )
      if (!hasTags) {
        this.loggerGlobal.warn(
          {
      ...
      if (phone === undefined && email === undefined) {
        ...
        throw new ApiWarning(
          '[BossWebhooksService: processPeopleUpsert]: No email or phone found'
        )
      }
      ...
      if (!client) {
        ...
        debug(
          '[Worker: processPeopleUpsert]: updated client: %O',
          updatedClient
        )
      }
  ...
  async processPeopleDelete(params: BossPeopleWebhookDeleteDto) {
    ...
    try {
      ...
      if (!client) {
        this.loggerGlobal.warn(
          '[BossWebhooksService: processPeopleDelete]: Missing client on Repliers side with externalId: %s',
          payload.id.toString()
        )
  ...
  private async makeSureAVMLink(
    ...
    const link = secureFubAvmLink(
      this.config.eventsCollection.clientUrl,
      client.clientId.toString(),
      this.config.auth.agents_signature_salt
    )
    const existingValue = fieldName ? person[fieldName] : null
    if (
      !_.isString(existingValue) ||
      _.isEmpty(existingValue) ||
      existingValue !== link
    ) {
      debug(
        `[Worker: makeSureAVMLink]: existing value: "${existingValue}" updating link to "${link}"`
      )
      this.loggerGlobal.info(
        {
          data: {
      ...
      debug(
        `[Worker: makeSureAVMLink]: no update needed, existing value "${existingValue}" is up to date.`
      )
    }
  }
}
