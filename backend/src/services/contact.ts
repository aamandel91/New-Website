...
import {
  ContactContactUsDto,
  ContactEstimateScheduleDto,
  ContactRequestInfoDto,
  ContactScheduleDto
...
export default class ContactService {
  constructor(
    ...
    private selectScheduleEstimateNoteParams: SelectScheduleEstimateNoteParams,
    private eventsCollectionService: EventsCollectionService,
    private baseEstimateSelector: BaseEventCollectionSelector,
  ...
  async scheduleEstimate(params: ContactEstimateScheduleDto) {
    ...
    const clientUrl = this.config.eventsCollection.clientUrl.replace(
      '[CLIENT_ID]',
      defaults.clientId.toString()
    )
    const estimate = await this.repliers.estimate
      .get({
        estimateId: +params.estimateId
      })
      .then(({ estimates }) => estimates[0])
    if (!estimate) {
    ...
    if (!params.clientId) {
      ...
      this.eventsCollectionService.eventsCreate({
        ...
        property: {
          street: estimate.payload?.address
            ? this.baseEstimateSelector.addressShort(estimate.payload?.address)
            : undefined,
  ...
  private async defaultMessageFields(params: ContactDTO) {
    const clientId =
      params?.clientId || this.config.repliers.clients.unauthenticatedClientId
    const agentId = params.clientId
      ? (await this.repliers.clients.get(params.clientId)).agentId
      : this.config.repliers.clients.defaultAgentId
    return {
      agentId,
      clientId,
      sender: 'client'
  ...
  private async sendEmail(options: RplMessagesSendRequest) {
    if (
      ...
      this.config.repliers.clients.defaultAgentEmail
    ) {
      return this.smtp.send({
        ...
        text:
          ...
          `You have a new message from a client. ${options.clientId}`
      })
    }
