...
import RepliersListings, {
  rplListingClassToRplClassMapper,
  RplListingsSearchRequest,
  RplListingsSingle
} from './repliers/listings.js'
import {
  DeleteEstimateDto,
  EstimateGetDto,
  EstimatePropertyDetailsDto,
  EstimatesByClientIdGetDto,
  RplEstimatePatchDto
} from '../validate/estimate.js'
import { RplSortBy, RplStatus, RplYesNo } from '../types/repliers.js'
import RepliersEstimate, {
  RplEstimateAddDto,
  RplEstimateAddData,
  RplEstimateAddRequest,
  RplEstimateAddResponse,
  RplEstimateCore,
  RplEstimateSingle
} from './repliers/estimate.js'
...
type PropertyDetailsListingResponse = Partial<
  RplListingsSingle &
    RplEstimateAddDto['data'] & {
      similarity: number | undefined
    }
>
...
export function ownerAdjustedEstimates(
  estimate: number,
  confidence: number,
  ...
  if (data && data.improvements) {
    ...
    const adjustedEstimate =
      estimate +
      maintenanceSpent * 0.6 +
      improvementSpent * 0.8 +
      landscapingSpent * 0.7
    return {
...
const isUser = (params: RplEstimateAddDto): boolean =>
  !!('clientId' in params && params.clientId)
@injectable()
export default class EstimateService {
  private propertyDetailsFields = [
    'class',
    'address',
    'images[0]',
    'listDate',
  ...
  constructor(
    ...
    private listings: RepliersListings,
    private historicalListings: RepliersListings,
    private estimate: RepliersEstimate,
    private usersService: UserService
  ) {
    if (this.config.repliers.historical_data_key) {
      this.historicalListings.switchKey(
        this.config.repliers.historical_data_key
      )
      this.historicalServiceEnabled = true
    }
  }
  async propertyDetails(
    params: EstimatePropertyDetailsDto
  ): Promise<PropertyDetailsListingResponse | PropertyDetailsEmptyResponse> {
    ...
    const listing = this.shouldAdjustTax(foundListing)
      ? await this.adjustPropertyTax(foundListing)
      : foundListing
    const { mostSimilar = undefined, similarity = 0 } = isUser(params)
      ? await this.findSimilarClientEstimate(params)
      : {}
    if (similarity < 0.9) {
      return listing
    }
  ...
  private async findListingByAddress(
    params: EstimatePropertyDetailsDto
  ): Promise<RplListingsSingle | undefined> {
    const searchParams = {
      listings: true,
    ...
    return this.historicalServiceEnabled
      ? this.fetchFromHistory(searchParams)
      : undefined
  }
  private async fetchFromHistory(
    searchParams: RplListingsSearchRequest
  ): Promise<RplListingsSingle | undefined> {
    const historicalResponse =
      await this.historicalListings.search(searchParams)
    debug(
      '[propertyDetails] historicalResponse.listings %O',
      historicalResponse.listings
    )
  ...
  private shouldAdjustTax(listing: RplListingsSingle): boolean {
    ...
    return (
      listingTaxYear !== new Date().getFullYear() ||
      listingTaxYear !== this.getCurrentTaxYear()
    )
  }
  ...
  private getListingTaxYear(listing: RplListingsSingle): number {
    const assessmentYear =
      listing.taxes.assessmentYear || listing.listDate || listing.updatedOn
    return new Date(assessmentYear).getFullYear()
  }
  private async adjustPropertyTax(
    listing: RplListingsSingle
  ): Promise<RplListingsSingle> {
    ...
    try {
      debug(
        '[propertyDetails] adjustPropertyTax fetching tax stats %O',
        searchParams
      )
      const response = await (this.historicalServiceEnabled
        ? this.historicalListings.search(searchParams)
        : this.listings.search(searchParams))
      debug(
        '[propertyDetails] adjustPropertyTax fetched tax stats %O',
        response
      )
      const { statistics } = response
      const listingYearStats = statistics.tax.yr?.[`${listingTaxYear}`]
      const currentYearStats = statistics.tax.yr?.[`${currentTaxYear}`]
      const noListingYearStats =
        !listingYearStats ||
        listingYearStats.count < MINIMUM_REQUIRED_RECORDS ||
        !listingYearStats[statKey]
      const noCurrentYearStats =
        !currentYearStats ||
        currentYearStats.count < MINIMUM_REQUIRED_RECORDS ||
        !currentYearStats[statKey]
      ...
      listing.taxes.annualAmount = Math.round(
        listing.taxes.annualAmount * taxMultiplier
      )
      return listing
    } catch (err) {
  ...
  private findClosestLocation(
    params: EstimatePropertyDetailsDto,
    listings: RplListingsSingle[]
  ): RplListingsSingle | null {
    let closest = null
    ...
    debug(
      '[propertyDetails] findClosestLocation in %O listings',
      listings.length
    )
    for (const listing of listings) {
      if (
        !listing.address['streetNumber'] ||
        isNaN(+listing.address['streetNumber'])
      ) {
        continue
      }
      ...
      if (isSameParity && distance <= minimumSatisfactoryDistance) {
        debug(
          '[propertyDetails] findClosestLocation found minimumSatisfactoryDistance %0',
          listing
        )
        return listing
      }
    ...
    if (closestSameParity) {
      debug(
        '[propertyDetails] findClosestLocation found same parity %0',
        closestSameParity
      )
  ...
  private async getAverageTax({
    ...
    const search = {
      ...
      minListDate: dayjs()
        .subtract(12, 'months')
        .format('YYYY-MM-DD') as `${number}-${number}-${number}`,
      statistics: 'med-tax'
    }
    try {
  ...
  private async findSimilarClientEstimate(
    params: EstimatePropertyDetailsDto
  ): Promise<SimilarClientEstimate> {
    const { estimates: prevEstimates } = await this.estimate.get({
    ...
    const mostSimilar = prevEstimates
      .sort((a, b) => {
        return howSimilar(simParams, a.payload.address!) >
          howSimilar(simParams, b.payload.address!)
          ? -1
          : 1
      })
      .at(0)
    const similarity = mostSimilar
      ? howSimilar(simParams, mostSimilar.payload.address!)
      : undefined
    return {
      mostSimilar,
      similarity
    ...
    const repliersParams: RplEstimateAddRequest = {
      ...
      ...(isUser(params)
        ? {
            ...
            sendEmailNow:
              (params.sendEmailNow ||
                this.config.repliers.estimatesNotificationSettings
                  ?.sendEmailNow) ??
              false,
            sendEmailMonthly:
              (params.sendEmailMonthly ||
                this.config.repliers.estimatesNotificationSettings
                  ?.sendEmailMonthly) ??
              false
          }
  ...
  async get(params: EstimateGetDto) {
    ...
    const { clientId: _clientId, ...requestParams } = params
    const { estimates } = await this.estimate.get(requestParams)
  ...
  private adjustHistory(
    result: RplEstimateCore /*RplEstimateSingle | RplEstimateAddResponse*/,
    delta: number
  ) {
  ...
  private adjustEstimates<T extends RplEstimateCore>(
    result: T,
    data: RplEstimateAddData | undefined
  ) {
    const unadjustedEstimate = result.estimate
    const adjustedEstimates = ownerAdjustedEstimates(
      result.estimate,
      result.confidence,
      data
    )
  ...
  async getByClientId(params: EstimatesByClientIdGetDto) {
    ...
    const { estimates } = await this.estimate.get({
      clientId
    })
  ...
  async checkOwnership(params: CheckOwnershipParams): Promise<boolean> {
    ...
    const { estimateId, userId, userRole } = params
    const estimate =
      params.estimate ??
      (await this.get({
