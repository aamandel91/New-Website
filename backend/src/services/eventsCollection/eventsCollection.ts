...
export type NotesCollectionPropertiesSelector = (
  ctx: Context
) => Promise<BossNoteCreateRequest | null>
@injectable()
export default class EventsCollectionService {
  ...
  eventsCreate(params: Partial<BossEventsCreateRequest>) {
    const { ignoreDefaultTags, ...rest } = params
    ...
    this.boss
      .eventsCreate({
        ...
        person: {
          ...
          tags: ignoreDefaultTags
            ? params.person?.tags
            : this.assignTags(params.person)
        }
      })
      .then((r) => {
  ...
  async noteCreate(params: BossNoteCreateRequest) {
    ...
    const { clientId, personId, ...rest } = params
    this.getPersonId({
      clientId,
      personId
    }).then((personId) => {
        ...
        this.userService.info(+params.clientId).then(async (client) => {
          if (client.externalId) {
            ...
            if (bossPersons?.people?.[0]?.id) {
              ...
              reject(
                new Error(`Person not found for clientId: ${params.clientId}`)
              )
            }
          }
        })
