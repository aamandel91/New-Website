...
import {
  RplEstimateAddDto,
  RplEstimateCore
} from '../../../services/repliers/estimate.js'
const debug = _debug('repliers:services:BaseEventCollectionSelector')
export type EventsCollectionPropertiesSelector = (
  ctx: Context
) => Promise<BossEventsCreateRequest | null>
@injectable()
export default class BaseEventCollectionSelector {
  constructor(
    protected repliers: RepliersService,
    protected agentsService: RepliersAgents,
  ...
  async getPerson(
    email: string,
    defaults: Partial<BossEventPerson> = {}
  ): Promise<BossEventPerson> {
  ...
  buildPropertyUrl(mlsNumber: number | string, boardId?: number) {
    if (boardId) {
      return this.config.eventsCollection.propertyUrl
        .replace('[MLS_NUMBER]', mlsNumber.toString())
        .replace('[BOARD_ID]', boardId.toString())
    }
    return this.config.eventsCollection.propertyUrl
      .replace('[MLS_NUMBER]', mlsNumber.toString())
      .replace('boardId=[BOARD_ID]', '')
  }
  ...
  addressShort(address: RplListingsSingleResponse['address']) {
    const { streetNumber, streetName, streetSuffix, unitNumber } = address
    const formattedUnitNumber = unitNumber ? `#${unitNumber} - ` : ''
    const formattedStreetNumber = this.sanitizedStreetNumber(
  ...
  sanitizedStreetNumber(streetNumber: string) {
    if (streetNumber === '00' || streetNumber === '0' || !streetNumber)
      return ''
    return streetNumber
  }
  ...
  mapRplPropertySearchToBoss(
    propertySearch: RplSearchesCreateDto
  ): BossEventPropertySearch {
    return {
  ...
  async getAgent(agentId: number): Promise<{
    ...
    const { users } = await this.bossService.getUsers({
      name: agentName
    })
    return users[0]
      ? {
  ...
  getSellIntentionTag(estimate: RplEstimateAddDto): string[] {
    ...
    const sellingTimeline = estimate.data?.salesIntentions
      ?.sellingTimeline as keyof typeof tagsMap
  ...
  getEstimateUrl(responseBody: unknown) {
    if (this.isEstimateModel(responseBody)) {
      const useUlid =
        this.config.eventsCollection.estimateUrl.includes('[ULID]')
      const value = useUlid
        ? responseBody.ulid
        : responseBody.estimateId.toString()
