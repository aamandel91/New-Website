...
import BaseEventCollectionSelector, {
  EventsCollectionPropertiesSelector
} from './baseEventCollectionSelector.js'
import {
  AgentCreateEstimateDto,
...
export default class SelectAgentEstimateParams extends BaseEventCollectionSelector {
  ...
  select: EventsCollectionPropertiesSelector = async (ctx) => {
    try {
      ...
      const { address, details } = value
      const price = this.isEstimateModel(ctx.response.body)
      ...
      return {
        ...
        person: {
          ...
          tags: [
            ...this.getSellIntentionTag(value),
            ...(defaults.person.tags || [])
          ],
          ...agentProps
        },
        property: {
