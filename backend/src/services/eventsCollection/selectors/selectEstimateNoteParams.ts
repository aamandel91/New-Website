...
export default class SelectEstimateNoteParams {
  constructor(
    private baseEventCollectionSelector: BaseEventCollectionSelector
  ) {}
  select: NotesCollectionPropertiesSelector = async (ctx) => {
    ...
    try {
      ...
      const estimateUrl = this.baseEventCollectionSelector.getEstimateUrl(
        ctx.response.body
      )
      if (!estimateUrl) {
        debug(
          'Estimate URL is missing for the provided estimate',
          ctx.response.body
        )
      }
      return {
  ...
  private getEstimatedPropertyNoteOptions(estimate: unknown) {
    ...
    if (
      ...
      !estimateDetails.clientId
    ) {
      debug('Invalid estimate format', estimate)
      return null
    }
    const { address, details, clientId } = estimateDetails
    return {
      clientId,
      ...
      bedrooms: this.baseEventCollectionSelector.stringifyIfSet(
        details.numBedrooms
      ),
      bathrooms: this.baseEventCollectionSelector.stringifyIfSet(
        details.numBathrooms
      ),
      yearBuilt: this.baseEventCollectionSelector.stringifyIfSet(
        details.yearBuilt
      ),
      garageSpaces: this.baseEventCollectionSelector.stringifyIfSet(
  ...
  private isRplEstimateSingle(
    estimate: unknown
  ): estimate is RplEstimateSingle {
    return (estimate as RplEstimateSingle).payload !== undefined
  }
  private isRplEstimateAddResponse(
    estimate: unknown
  ): estimate is RplEstimateAddResponse {
