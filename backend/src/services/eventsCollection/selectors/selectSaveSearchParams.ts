...
export default class SelectSaveSearchParams extends BaseEventCollectionSelector {
  ...
  getSaveSearchUrl(responseBody: unknown) {
    if (
      this.isSearchModel(responseBody) &&
      this.config.eventsCollection.savedSearchUrl
    ) {
      return this.config.eventsCollection.savedSearchUrl.replace(
        '[SEARCH_ID]',
        responseBody.searchId.toString()
      )
    }
    return null
  }
