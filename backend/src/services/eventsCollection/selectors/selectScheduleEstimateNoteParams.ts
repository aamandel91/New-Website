...
export default class SelectScheduleEstimateNoteParams {
  ...
  select = async (
    params: ScheduleEstimateNoteParams
  ): Promise<BossNoteCreateRequest> => {
    debug('[SelectEstimateNoteParams] estimateParams %O', params)
    const { estimate, date, time } = params
    const addressShort = estimate.payload?.address
      ? this.baseEventCollectionSelector.addressShort(estimate.payload.address)
      : 'unknown address'
    const price = this.baseEventCollectionSelector.formatPrice(
      estimate?.estimate
    )
    const estimateUrl =
      this.baseEventCollectionSelector.getEstimateUrl(estimate)
