...
import type {
  RplListingsCountDto,
  RplListingsLocationsDto,
  RplListingsSearchDto,
  RplListingsSimilarDto,
  RplListingsSingleDto,
  RplNlpDto
} from '../validate/listings.js'
import {
  RplArea,
  RplCity,
  RplListingsLocationsResponse,
  RplListingsSingleResponse,
  RplNeighborhood
...
// HELPERS
const allowedAreas = (areas: RplArea[]) =>
  config.settings.locations.allow_all_areas
    ? areas
    : areas.filter((area) =>
        config.settings.locations.allowed_areas.includes(
          area.name.toLowerCase()
        )
      )
const locationClassIdx = (
  locations: RplListingsLocationsResponse,
  rplClass: RplClass
) => locations.boards[0].classes.findIndex((cls) => cls.name === rplClass)
const locationAreas = (
  locations: RplListingsLocationsResponse,
  rplClass: RplClass
) => locations.boards[0].classes[locationClassIdx(locations, rplClass)]!.areas
@injectable()
export default class ListingsService {
  constructor(
    private repliers: RepliersService,
    @inject('config')
    private config: AppConfig
  ) {}
  private ensureRequiredFields(params: { fields?: string }) {
    let { fields = '' } = params
  ...
  async search(params: RplListingsSearchDto) {
    const { app_state: _state, ...rplParams } = params
  ...
  private validateAvailability(listing: RplListingsSingleResponse) {
    const hiddenStatuses =
      this.config.settings.hide_unavailable_listings_statuses
    ...
    if (hiddenStatuses.includes(status)) {
      debug(
        `[ListingsService: single]: %0 has status %1 and cannot be returned. HiddenStatuses are %2`,
        listing.mlsNumber,
        status,
        hiddenStatuses
      )
  ...
  private cityExist(
    area_name: string,
    city_name: string,
    residentalAreas: RplArea[]
  ) {
    const areaIdx = residentalAreas.findIndex((area) => area.name === area_name)
    return residentalAreas[areaIdx]?.cities.find(
      (city) => city.name === city_name
    )
  }
  private neighborhoodExist(
    area_name: string,
    city_name: string,
  ...
  private copyNeighborhood(
    area_name: string,
    city_name: string,
    neighborhood: RplNeighborhood,
    residentalAreas: RplArea[]
  ) {
    const areaIdx = residentalAreas.findIndex((area) => area.name === area_name)
    ...
    const cityIdx = residentalAreas[areaIdx]?.cities.findIndex(
      (city) => city.name === city_name
    )
    ...
    debug(
      'Copy neighborhood, area: %s, city: %s, neighborhood: %o ',
      area_name,
      city_name,
      neighborhood
    )
  ...
  // by ref
  private removeEmptyNeighborhoods(
  ...
  // by ref
  private removeEmptyLocations(
    locations: RplListingsLocationsResponse,
    limit: number = 0
  ) {
    const residentalIdx = locations.boards[0].classes.findIndex(
      (cls) => cls.name === RplClass.residential
    )
  ...
  async locations(
    params: RplListingsLocationsDto
  ): Promise<
    RplListingsLocationsResponse | Cached<RplListingsLocationsResponse>
  > {
    const { dropCoordinates, ...repliersParams } = params
    ...
    // by ref
    const allCondoAreas =
      -1 < locationClassIdx(locations, RplClass.condo)
        ? locationAreas(locations, RplClass.condo)
        : []
    const allResidentalAreas =
      -1 < locationClassIdx(locations, RplClass.residential)
        ? locationAreas(locations, RplClass.residential)
        : []
    ...
    if (-1 < locationClassIdx(locations, RplClass.condo)) {
      locations.boards[0].classes.splice(
        locationClassIdx(locations, RplClass.condo),
        1
      )
    }
    ...
    if (-1 < locationClassIdx(locations, RplClass.residential)) {
      this.removeExtraAreas(
        locations,
        locationClassIdx(locations, RplClass.residential)
      )
    }
