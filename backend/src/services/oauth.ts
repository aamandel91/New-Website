...
export default class OAuthService {
  ...
  public async callback(
    ...
    if (!tokens.id_token) {
      ...
      throw new ApiError(
        `No id_token recieved from ${provider}, probably insufficient scopes configured for app`,
        400
      )
    }
    ...
    if (maybeClients.clients[0] === undefined) {
      ...
      const maybeAgent = this.getAgentWithEmail(
        userInfo['email'],
        maybeAgents.agents
      )
      if (maybeAgent) {
        ...
        const userProfile = await this.adapters[provider].extractUserInfo(
          userInfo,
          tokens.access_token
        )
  ...
  private async reportClientRegistration(
    ...
    const trafficSource = await this.trafficSourceService.getTrafficSource(
      user.clientId
    )

    let trafficSourceForReport:
      | {
          utmSource?: string
          utmMedium?: string
          utmCampaign?: string
    ...
    if (trafficSource) {
      ...
      if (trafficSource.utmSource !== undefined)
        trafficSourceForReport.utmSource = trafficSource.utmSource
      if (trafficSource.utmMedium !== undefined)
        trafficSourceForReport.utmMedium = trafficSource.utmMedium
      if (trafficSource.utmCampaign !== undefined)
        trafficSourceForReport.utmCampaign = trafficSource.utmCampaign
      if (trafficSource.trafficType !== undefined)
        trafficSourceForReport.trafficType = trafficSource.trafficType
      if (trafficSource.landingPage !== undefined)
        trafficSourceForReport.landingPage = trafficSource.landingPage
    }
    const selectInput: any = { user, provider }
    if (trafficSourceForReport !== undefined)
      selectInput.trafficSource = trafficSourceForReport
