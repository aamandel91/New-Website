...
interface RplEsitmatePatchRequest
  extends Omit<RplEstimateAddRequest, 'boardId'> {
  estimateId: number
}
@injectable()
export default class RepliersEstimate extends RepliersBase {
  add(params: RplEstimateAddRequest) {
    return this.request<RplEstimateAddResponse | RplEstimateSingle>(
      'POST',
      `/estimates`,
      {},
      params
  ...
  delete(params: RplEstimateDeleteResponse) {
    return this.request<RplEstimateGetResponse>(
      'DELETE',
      `/estimates/${params.estimateId}`
    )
  }
  patch(params: RplEsitmatePatchRequest) {
    const { estimateId, ...data } = params
    return this.request<RplEstimateGetResponse>(
      'PATCH',
      `/estimates/${params.estimateId}`,
      {},
