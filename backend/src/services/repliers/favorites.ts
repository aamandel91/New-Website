...
export interface RplFavoritesAddRequest
  extends RplFavoritesAddDto,
    ApiRequest {}
...
export default class RepliersFavorites extends RepliersBase {
  add(params: RplFavoritesAddRequest) {
    return this.request<RplFavoritesAddResponse>(
      'POST',
      `/favorites`,
      {},
      params
  ...
  delete(favoriteId: number) {
    return this.request<RplFavoritesDeleteResponse>(
      'DELETE',
