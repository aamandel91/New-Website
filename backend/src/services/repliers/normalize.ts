...
export default class RepliersNormalize extends RepliersBase {
  public async normalizeRETS(listings: RplListingsSingleRETS[]) {
    return this.request<NormalizeResponse>(
      'POST',
      `/property/normalize`,
      undefined,
      {
  ...
  public mapNormalizedData(
    data: NormalizedData,
    listing: RplListingsSingleRETS
  ): RplListingsSingle {
    ...
    const details = {
      ...
      basement2: null,
      exteriorConstruction1: data.ConstructionMaterials
        ? data.ConstructionMaterials.join(',')
        : null,
