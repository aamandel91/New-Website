...
export class ListingsScrubber extends BaseScrubber {
  ...
  private asGuest(
    data: RequiredFields<Partial<RplListingsSingle>, ImportantFields>
  ) {
    ...
    if (
      ...
      (!Array.isArray(data['duplicates']) ||
        !data['duplicates'].includes(config.settings.scrubbing_ref_board_id)) // ( if no duplicates or duplicates is array which doesn't have CREA_BOARD_RESOURCE_ID number )
    ) {
      data['permissions'].displayPublic = RplYesNo.N // treat as disaplyPublic = N
    }
    if (
      data['status'] === RplStatus.U ||
      data['permissions'].displayPublic === RplYesNo.N
    ) {
      if ('address' in data) {
        data['address'] = this.objectScrubber(
          data['address'],
          SafeAddressFields
        )
      }
    ...
    if ('history' in data) {
      data.history = data.history.map((item) => {
        if ('address' in item) {
          item['address'] = this.objectScrubber(
            item['address'],
            SafeAddressFields
          )
        }
  ...
  extend(
    data: Partial<RplListingsSingle>,
    fields: Array<ImportantFields>,
    source: RequiredFields<Partial<RplListingsSingle>, ImportantFields>
  ) {
  ...
  public mapLastStatus(item: { status?: RplStatus; lastStatus?: string }) {
    const { status, lastStatus } = item
    if (
      status === RplStatus.A ||
      (lastStatus !== undefined &&
        ['New', 'Sc', 'Sce', 'Lc', 'Pc', 'Ext', 'Lce', 'Dft'].includes(
          lastStatus
        ))
    ) {
      return RplStatus.A
  ...
  return (
    ...
    return {
      get() {
        ...
        const wrapperFn = (
          ...args: any[] | [ScrubberFunctionParams, ...any[]]
        ) => {
          return propertyDescriptor.value
            ...
            .then((result: any) => {
              ...
              const scrubbedData = scrubbingArray.map(
                scrubber.scrub.bind(scrubber)
              )
              return key
                ? {
                    ...result,
