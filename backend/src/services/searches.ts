...
export default class SearchesService {
  ...
  async delete(params: { searchId: number; clientId: number }) {
    const response = await this.getAll({
      clientId: params.clientId
    })
    const isOwned = response.searches.find(
      (search) => search.searchId === params.searchId
    )
  ...
  async getOne(searchId: number) {
    ...
    const { client: _, agentId: __, ...rest } = result
    return rest
  }
}
