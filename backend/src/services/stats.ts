...
import type {
  StatsCommunitiesDto,
  StatsNeighborhoodsrankingDto,
  StatsWidgetsDto
} from '../validate/stats.js'
import RepliersListings, {
  RplListingsSearchResponse,
  RplRollingPeriodName,
  StatsMoment
...
export default class StatsService {
  constructor(
    private repliers: RepliersListings,
    @inject('data.communities')
    private data: DataCommunities
  ) {}
  getSum<T extends string>(
    records: Record<string, Record<T, number>> | undefined,
    field: T
  ) {
    if (!records) {
      return 0
  ...
  async widgets(
    params: StatsWidgetsDto
  ): Promise<WidgetsReturn | Cached<WidgetsReturn>> {
    const periods = {
      month: dayjs()
        .utc()
        .subtract(1, 'month')
        .format(RplDateFormatter) as RplDateFormat,
      threeMonths: dayjs()
        .utc()
        .subtract(3, 'month')
        .format(RplDateFormatter) as RplDateFormat,
      year: dayjs()
        ...
        .subtract(1, 'year')
        .format(RplDateFormatter) as RplDateFormat,
      historyMinDate: dayjs()
        .utc()
        .subtract(params.historyMonthsCount, 'month')
        .startOf('month')
        .format(RplDateFormatter) as RplDateFormat
    }
    const months = _.range(0, params.historyMonthsCount).map((i) => {
      return dayjs()
        ...
        .subtract(i, 'month')
        .startOf('month')
        .format('YYYY-MM') as RplMonthFormat
    })
    const { map, area, city, neighborhood, community } = params
    const district =
      0 < community?.length ? this.getDistrictIds(community) : params.district
    const widgetParams = {
      map,
      class: params.class,
      area,
    ...
    const statParams = {
      ...widgetParams,
      statistics:
        'avg-soldPrice,med-soldPrice,sum-soldPrice,avg-daysOnMarket,med-daysOnMarket',
      status: [RplStatus.U]
    }
    const [
      monthResult,
    ...
    const newListingsCount = months.reduce(
      (
        acc: Record<
          RplMonthFormat,
          {
    ...
    const soldPerMonthCount = months.reduce(
      (
        acc: Record<
          RplMonthFormat,
          {
    ...
    const soldPerMonthMoment = months.reduce(
      (acc: Record<RplMonthFormat, StatsMoment>, month) => {
        acc[month] = {
  ...
  calcPeriodAvg(
    stats: RplListingsSearchResponse,
    key: RplRollingPeriodName,
    currPeriodStart: string,
    ...
    debug(
      `stats.statistics.soldPrice[${key}]:`,
      stats.statistics.soldPrice[key]
    )
  ...
  async fetchStats(
    params: Pick<StatsNeighborhoodsrankingDto, 'class'>
  ): Promise<FetchStatsReturn | Cached<FetchStatsReturn>> {
    const nbList = Object.entries(this.data.communities)
    return Promise.all(
      nbList.map(async ([neighborhood, district]) => {
        ...
        const lastMonth = dayjs()
          .utc()
          .subtract(30, 'days')
          .format(RplDateFormatter)
        const lastMonthPrev = dayjs()
          .utc()
          .subtract(60, 'days')
          .format(RplDateFormatter)
        const last3Month = dayjs()
          .utc()
          .subtract(90, 'days')
          .format(RplDateFormatter)
        const last3MonthPrev = dayjs()
          .utc()
          .subtract(180, 'days')
          .format(RplDateFormatter)
