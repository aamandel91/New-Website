...
import {
  UserSocialCallbackDto,
  UserSocialGetTokensDto,
  UserSocialRefreshDto,
  UserSocialUnlinkDto
...
export default class SocialService {
  ...
  async callback(req: IncomingMessage, params: UserSocialCallbackDto) {
    ...
    await this.socialRepository.createTokens(
      params.email,
      params.provider,
      tokens
    )
  ...
  async refresh(params: UserSocialRefreshDto) {
    const tokenInfo = await this.socialRepository.getUserTokens(
      params.email,
      params.provider
    )
    if (!tokenInfo) {
    ...
    return this.socialRepository.updateAccessToken(
      params.email,
      params.provider,
