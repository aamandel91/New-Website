...
import {
  RplClientsCreateDto,
  RplClientsFilterRequest,
  RplClientsUpdateRequest
} from '../services/repliers/clients.js'
import {
  RplMessagesGetDto,
  RplMessagesSendDto
} from '../services/repliers/messages.js'
...
export interface AgentUpdateEstimateDto
  extends Omit<RplEstimateAddDto, 'boardId'> {
  agentId: number
  estimateId: number
}
export const agentsUpdateEstimateSchema: joi.ObjectSchema<AgentUpdateEstimateDto> =
  estimateBaseSchema.keys({
...
export const agentsCreateClientSchema = joi.object<AgentCreateClientDto>(
  agentsCreateClientSchemaKeys
)
export const agentsCreateEstimateSchema = (
  estimateAddSchema as joi.ObjectSchema<AgentCreateEstimateDto>
).keys({
  agentId: agentIdSchema
})
...
export const agentsSendEstimateSchema = joi
  .object<AgentSendEstimateDto>()
  .keys({
    agentId: agentIdSchema,
    estimateId: joi.number().required()
  })
...
export const agentGetBossPeopleSchema = joi.object<AgentGetBossPeopleDto>({
  assignedUserId: joi.number().required().messages({
    'any.required':
      'You need an account in FollowUpBoss, contact administroator to create one'
  }),
  agentId: agentIdSchema,
  tags: joi.string()
})
...
export const agentUpdateClientSchema = (
  agentsCreateClientSchema.fork(
    Object.keys(agentsCreateClientSchemaKeys),
    (schema) => schema.optional()
  ) as joi.ObjectSchema<AgentUpdateClientDto>
).keys({
  clientId: joi.number().required()
})
