...
const baseAutosuggestSchema = {
  lat: joi
    .string()
    .regex(/^(-?\d+(\.\d+)?)$/)
    .default(config.repliers.autosuggest.lat),
  long: joi
    .string()
    .regex(/^(-?\d+(\.\d+)?)$/)
    .default(config.repliers.autosuggest.long),
  mapboxSearchSession: joi.string().uuid(),
  searchSession: joi.string().uuid()
}
export const autosuggestSchema = joi
  .object<AutosuggestDto>()
  .keys({
    ...
    resultsPerPage: joi
      .number()
      .positive()
      .max(config.repliers.autosuggest.max_results)
      .default(config.repliers.autosuggest.max_results_default)
  })
  .or('mapboxSearchSession', 'searchSession')
export const autosuggestAddressSchema = joi
  .object<AutosuggestAddressDto>()
  .keys({
    ...baseAutosuggestSchema,
    q: joi.string().min(1),
