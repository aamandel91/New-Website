import joi from 'joi'
import {
  RplEstimateAddCondominium,
  RplEstimateAddDto,
  RplEstimateAddData,
  RplEstimateAddDataImprovements,
  RplEstimateAddDataSalesIntentions,
  RplEstimateMortgage
...
const estimateStringOrArraySchema = joi.alternatives(
  joi.string().max(70),
  joi.array().items(joi.string().max(70)).max(10)
)
export const estimateBaseSchema = joi.object().keys({
  ...
  details: joi.object<RplEstimateAddDto['details']>().keys({
    ...
    yearBuilt: joi.alternatives(
      joi.number().integer().positive().allow(''),
      joi.string().allow('').max(8)
    )
  }),
...
export const estimateAddSchema: joi.ObjectSchema<RplEstimateAddDto> =
  estimateBaseSchema.keys({
    boardId: joi.number().positive().integer().required()
  })
export interface RplEstimatePatchDto
  extends Omit<RplEstimateAddDto, 'boardId'> {
  estimateId: number
  clientId: number
}
export const estimatePatchSchema: joi.ObjectSchema<RplEstimatePatchDto> =
  estimateBaseSchema.keys({
...
export const estimateGetSchema = joi
  .object<EstimateGetDto>()
  .keys({
...
export const estimatePropertyDetailsSchema = joi
  .object<EstimatePropertyDetailsDto>()
  .keys({
...
export const estimatesByClientIdGetSchema = joi
  .object<EstimatesByClientIdGetDto>()
  .keys({
    clientId: joi.number().required()
  })
