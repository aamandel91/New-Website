...
const listingsCountOmittedFields = [
  'fields',
  'app_state',
  'listings',
  'resultsPerPage',
...
export interface RplListingsCountDto
  extends Omit<
    RplListingsSearchDto,
    (typeof listingsCountOmittedFields)[number]
  > {}
const listingSearchSchemaKeys = {
  ...
  boardId: joi
    ...
    .items(joi.number())
    .single()
    .default(config.settings.defaults.boardId),
  ...
  class: joi
    .array()
    .items(joi.string().valid(...Object.keys(RplClass)))
    .single(),
  cluster: joi.boolean(),
...
export const listingSearchSchema = joi
  .object<RplListingsSearchDto>()
...
export const listingCountSchema = joi
  .object<RplListingsCountDto>()
...
export const listingsLocationsSchema = joi
  .object<RplListingsLocationsDto>()
  .keys({
    area: joi.string(),
    city: joi.string(),
