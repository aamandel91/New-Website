...
describe('Listings', function () {
  ...
  describe('Single listing', function () {
    const setupSingle = (
      mlsNumber: string,
      stub: Partial<RplListingsSingleResponse>
    ) => {
    ...
    it("should respond with `unavailable_listings_http_code` when listing's last status is Ter", async function () {
      ...
      const listing = await appInstance.get(
        '/api/listings/terminated?boardId=1'
      )
      assert.equal(
        listing.status,
        config.settings.hide_unavailable_listings_http_code
      )
    })
