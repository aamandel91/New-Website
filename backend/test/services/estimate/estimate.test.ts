...
describe('Estimates tests', () => {
  describe('Owner Adjusted Estimates tests', () => {
    ...
    describe('ownerAdjustedEstimate', () => {
      ...
      it('Correct Bounds with no History', () => {
        assert.deepEqual(
          ownerAdjustedEstimates(1000, 0.1, {
            improvements: {
  ...
  describe('propertyDetails', () => {
    ...
    describe('hood lookup', () => {
      it('Should return only city and tax details when no history as well as closest listings', async function () {
        ...
        mockEmptyAddressLookup()
        mockAverageTax()
        const propertyDetailsResponse = await appInstance.get(
          '/api/estimate/property_details?city=Toronto&streetName=Yonge&streetNumber=150&unitNumber=1'
        )
    ...
    describe('tax adjustment', () => {
      it('Should adjust property tax if old data', async () => {
        mockDate(new Date(2025, 5, 30))
        mockDynamicPropertyDetails(
          (queryObject) => queryObject.city === 'TaxAdjust',
          [
            {
              address: {
                city: 'Toronto'
        ...
        const propertyDetailsResponse = await appInstance.get(
          '/api/estimate/property_details?city=TaxAdjust&streetName=whatever&streetNumber=whocares'
        )
      ...
      it('Should not adjust property tax if recent data', async () => {
        ...
        const propertyDetailsResponse = await appInstance.get(
          '/api/estimate/property_details?city=RecentTax&streetName=whatever&streetNumber=whocares'
        )
      ...
      it('Should not adjust property tax if stats missing', async () => {
        ...
        const propertyDetailsResponse = await appInstance.get(
          '/api/estimate/property_details?city=NoStats&streetName=whatever&streetNumber=whocares'
        )
      ...
      it('Should return current year med tax if no tax in history', async () => {
        ...
        const propertyDetailsResponse = await appInstance.get(
          '/api/estimate/property_details?city=NoTax&streetName=whatever&streetNumber=whocares'
        )
