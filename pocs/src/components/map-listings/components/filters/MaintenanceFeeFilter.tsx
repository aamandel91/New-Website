...
export function MaintenanceFeeFilter({
      ...
      for (let i = 0; i < feeBreakpoints.length - 1; i++) {
        ...
        if (fee >= current && fee <= next) {
          ...
          const progressInSegment =
            next === Infinity ? 0 : (fee - current) / (next - current)
  ...
  const getFeeFromPosition = useCallback(
  ...
  const handleMouseMove = useCallback(
    (e: MouseEvent) => {
      ...
      if (isDragging === 'min') {
        ...
        const constrainedFee =
          newFee === null ? maxValue : Math.min(newFee, maxValue)
        setTempMinFee(constrainedFee === Infinity ? 0 : constrainedFee)
      } else if (isDragging === 'max') {
        const constrainedFee =
          newFee === null ? null : Math.max(newFee, tempMinFee)
    ...
    (handle: 'min' | 'max') => (e: React.KeyboardEvent) => {
      if (e.key === 'ArrowLeft' || e.key === 'ArrowRight') {
        ...
        if (handle === 'min') {
          ...
          const constrainedFee =
            newFee === null ? null : Math.max(newFee || 0, tempMinFee)
          setTempMaxFee(constrainedFee)
        }
      }
    },
      ...
      <div style={{ marginBottom: '16px' }}>
        ...
        >
          ...
          <div
            ...
            aria-valuemax={
              tempMaxFee === null
                ? feeBreakpoints[feeBreakpoints.length - 2]
                : tempMaxFee
            }
        ...
        >
          {feeBreakpoints.slice(0, -1).map((fee, index) => (
            <span key={index}>{formatDisplayFee(fee)}</span>
          ))}
          <span>Max</span>
        </div>
      </div>
