      ...
      for (let i = 0; i < priceBreakpoints.length - 1; i++) {
        ...
        if (price >= current && price <= next) {
          ...
          const progressInSegment =
            next === Infinity ? 0 : (price - current) / (next - current)
  ...
  const handleMouseMove = useCallback(
    (e: MouseEvent) => {
      ...
      if (isDragging === 'min') {
        ...
        const constrainedPrice =
          newPrice === null ? maxValue : Math.min(newPrice, maxValue)
        setTempMinPrice(constrainedPrice === Infinity ? 0 : constrainedPrice)
      } else if (isDragging === 'max') {
        const constrainedPrice =
          newPrice === null ? null : Math.max(newPrice, tempMinPrice)
    ...
    (handle: 'min' | 'max') => (e: React.KeyboardEvent) => {
      if (e.key === 'ArrowLeft' || e.key === 'ArrowRight') {
        ...
        if (handle === 'min') {
          ...
          const constrainedPrice =
            newPrice === null ? null : Math.max(newPrice || 0, tempMinPrice)
          setTempMaxPrice(constrainedPrice)
        }
      }
    },
      ...
      <div style={{ marginBottom: '20px' }}>
        ...
        >
          ...
          <div
            ...
            aria-valuemax={
              tempMaxPrice === null
                ? priceBreakpoints[priceBreakpoints.length - 2]
                : tempMaxPrice
            }
        ...
        >
          {priceBreakpoints.slice(0, -1).map((price, index) => (
            <span key={index}>{formatDisplayPrice(price)}</span>
          ))}
          <span>Max</span>
        </div>
      </div>
