...
export function SquareFootageFilter({
      ...
      for (let i = 0; i < sqftBreakpoints.length - 1; i++) {
        ...
        if (sqft >= current && sqft <= next) {
          ...
          const progressInSegment =
            next === Infinity ? 0 : (sqft - current) / (next - current)
  ...
  const getSqftFromPosition = useCallback(
  ...
  const handleMouseMove = useCallback(
    (e: MouseEvent) => {
      ...
      if (isDragging === 'min') {
        ...
        const constrainedSqft =
          newSqft === null ? maxValue : Math.min(newSqft, maxValue)
        setTempMinSqft(constrainedSqft === Infinity ? 0 : constrainedSqft)
      } else if (isDragging === 'max') {
        const constrainedSqft =
          newSqft === null ? null : Math.max(newSqft, tempMinSqft)
    ...
    (handle: 'min' | 'max') => (e: React.KeyboardEvent) => {
      if (e.key === 'ArrowLeft' || e.key === 'ArrowRight') {
        ...
        if (handle === 'min') {
          ...
          const constrainedSqft =
            newSqft === null ? null : Math.max(newSqft || 0, tempMinSqft)
          setTempMaxSqft(constrainedSqft)
        }
      }
    },
  ...
  return (
    ...
    >
      ...
      >
        {formatDisplaySqft(tempMinSqft)} - {formatDisplaySqft(tempMaxSqft)} sq
        ft
      </div>
      ...
      <div style={{ marginBottom: '16px' }}>
        ...
        >
          ...
          <div
            ...
            aria-valuemax={
              tempMaxSqft === null
                ? sqftBreakpoints[sqftBreakpoints.length - 2]
                : tempMaxSqft
            }
        ...
        >
          {sqftBreakpoints.slice(0, -1).map((sqft, index) => (
            <span key={index}>{formatDisplaySqft(sqft)}</span>
          ))}
          <span>Max</span>
        </div>
      </div>
