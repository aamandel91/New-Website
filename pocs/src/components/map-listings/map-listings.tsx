...
export function MapListings({
  ...
  const [clusterTooltipPosition, setClusterTooltipPosition] = useState({
    x: 0,
    y: 0
  })
  ...
  const fetchClustersRef = useRef<
    ((bounds: mapboxgl.LngLatBounds) => void) | null
  >(null)
  ...
  const addPriceMarkers = useCallback(
    (features: any[]) => {
      ...
      features.forEach((feature) => {
        if (
          ...
          const bubble = createPriceBubble(
            numericPrice,
            type,
            status,
            lastStatus
          ...
          bubble.addEventListener('click', async (event) => {
            ...
            try {
              const response = await fetch(
                `https://api.repliers.io/listings?mlsNumber=${feature.properties.mlsNumber}&fields=address.*,details.*,images,type,lastStatus,status,class`,
                {
                  headers: {
  ...
  const groupFeaturesByProximity = useCallback(
    (features: any[], radiusMeters: number): any[][] => {
      const groups: any[][] = []
      ...
      features.forEach((feature, index) => {
        ...
        features.forEach((otherFeature, otherIndex) => {
          ...
          const otherCoords = otherFeature.geometry.coordinates as [
            number,
            number
          ]
  ...
  const mergeSmallClusters = useCallback(
    (features: any[], zoom: number): any[] => {
      // Define what counts as a "small cluster" that should be merged
      // More aggressive merging at mid-zoom to reduce clutter
      const smallClusterThreshold =
        zoom <= 10 ? 30 : zoom <= 12 ? 60 : zoom <= 13 ? 100 : 30
      const mergeRadius =
        zoom <= 10 ? 500 : zoom <= 12 ? 300 : zoom <= 13 ? 200 : 100 // meters

      const smallClusters = features.filter(
        (f) => f.properties.count <= smallClusterThreshold
      )
      const largeClusters = features.filter(
        (f) => f.properties.count > smallClusterThreshold
      )
      ...
      const smallClusterGroups = groupFeaturesByProximity(
        smallClusters,
        mergeRadius
      )
      ...
      const mergedFeatures = smallClusterGroups.map((group) => {
        ...
        const totalCount = group.reduce(
          (sum, cluster) => sum + cluster.properties.count,
          0
        )
        const avgLng =
          group.reduce(
            (sum, cluster) => sum + cluster.geometry.coordinates[0],
            0
          ) / group.length
        const avgLat =
          group.reduce(
            (sum, cluster) => sum + cluster.geometry.coordinates[1],
            0
          ) / group.length
    ...
    async (bounds: mapboxgl.LngLatBounds) => {
      ...
      try {
        ...
        if (filters.activeListingDays !== undefined) {
          activeStatuses.push({
            type: 'active',
            value: filters.activeListingDays
          })
        }
        ...
        if (filters.unavailableListingDays !== undefined) {
          activeStatuses.push({
            type: 'unavailable',
            value: filters.unavailableListingDays
          })
        }
        ...
        if (needsBundledSearch) {
          ...
          console.log(
            `[Bundled Search] Multiple status filters active:`,
            activeStatuses.map((s) => s.type)
          )
          ...
          console.log(
            `[Bundled Search] POST Request Body:`,
          ...
          if (!response.ok) {
            ...
            throw new Error(
              `Bundled API Error: ${response.status} ${response.statusText} - ${errorText}`
            )
          }
          ...
          console.log(
            `[Bundled Search] API returned ${clusters.length} clusters and ${listings.length} listings (already merged)`
          )
        } else {
          ...
          console.log(
            `[Single Search] Status: ${statusType}, Value: ${statusValue}`
          )

          const url = new URL('https://api.repliers.io/listings')

          // Build query params for single status
          const queryParams = buildStatusQueryParams(
            statusType,
            statusValue,
            zoom
          )
        ...
        console.log(
          `   Clustering: ${zoom >= 18 ? 'DISABLED (individual listings only)' : 'ENABLED'}`
        )
        ...
        const mlsNumbers = rawListings
          .map((l: any) => l.mlsNumber)
          .filter(Boolean)
        const uniqueMlsNumbers = new Set(mlsNumbers)
        if (mlsNumbers.length !== uniqueMlsNumbers.size) {
          console.log(
            `   ⚠️ DUPLICATES DETECTED: ${mlsNumbers.length} total, ${uniqueMlsNumbers.size} unique (${mlsNumbers.length - uniqueMlsNumbers.size} duplicates)`
          )
        }
        ...
        const matchesPropertyTypeFilter = (
        ...
        clusters.forEach((cluster: any, index: number) => {
          ...
          if (cluster.count === 1 && zoom >= 14) {
            ...
            const clusterPropertyType =
              cluster.listing?.details?.propertyType ||
              cluster.details?.propertyType
                ...
                properties: {
                  ...
                  listPrice:
                    typeof listPrice === 'string'
                      ...
                      : listPrice,
                  propertyType:
                    cluster.propertyType ||
                    cluster.listing?.propertyType ||
                    cluster.status ||
                    'Sale',
        ...
        console.log(
          `   Processing ${rawListings.length} raw listings into property features...`
        )
        ...
        const propertyFeatures: PropertyFeature[] = rawListings
          .filter((listing: any) => {
            ...
            const matchesFilter = matchesPropertyTypeFilter(
        ...
        const recalculateClusterCounts = (
          clusters: ClusterFeature[],
          allProperties: PropertyFeature[],
          singleProps: PropertyFeature[]
        ) => {
          ...
          return clusters.map((cluster) => {
            const clusterCoords = cluster.geometry.coordinates as [
              number,
              number
            ]
        ...
        console.log(
          `   Single property features created: ${singlePropertyFeatures.length}`
        )
        console.log(`   Cluster features: ${clusterFeatures.length}`)
        if (rawListings.length > 0 && propertyFeatures.length === 0) {
          console.log(
            `   ⚠️ Filtering issue: ${filteredForDuplicate} duplicates, ${filteredForCoords} invalid coords, ${filteredForPropertyType} wrong property type`
          )
        }

        // Apply recalculation only when we have any properties (individual or single)
        const hasProperties =
          propertyFeatures.length > 0 || singlePropertyFeatures.length > 0
        const updatedClusterFeatures =
          zoom >= 14 && hasProperties
            ? recalculateClusterCounts(
                clusterFeatures,
                propertyFeatures,
        ...
        if (zoom >= 14 && hasProperties) {
          console.log(
            `   Recalculated cluster counts at zoom ${zoom.toFixed(2)}`
          )
        }
        ...
        if (zoom >= 14) {
          ...
          const proximityGroups = groupFeaturesByProximity(
            allPotentialFeatures,
            50
          )

          // For each proximity group, decide what to show
          allFeatures = proximityGroups
            .map((group) => {
              const clusters = group.filter((f) => f.type === 'cluster')
              const properties = group.filter(
                (f) => f.type === 'property' || f.type === 'singleProperty'
              )

              // Calculate total properties in clusters vs individual properties
              const clusterPropertyCount = clusters.reduce(
                (sum, c) => sum + (c.properties?.count || 0),
                0
              )
              ...
              if (properties.length > 3) {
                ...
                return clusters.length > 0
                  ? clusters.reduce((largest, current) =>
                      ...
                      (largest.properties?.count || 0)
                        ? current
                        : largest
                    )
                  : properties[0]
              } else if (
                clusterPropertyCount > individualPropertyCount &&
                clusters.length > 0
              ) {
          ...
          const mergedClusters = mergeSmallClusters(
            [...clusterFeatures, ...singlePropertyFeatures],
            zoom
          )
  ...
  useEffect(() => {
    ...
    map.current.on('load', () => {
      ...
      map.current.on('click', 'clusters', async (e) => {
        ...
        if (currentZoom >= 18) {
          console.log(
            `   ⚠️ Cluster exists at zoom 18+, zooming in further to break it apart`
          )
          map.current.easeTo({
            center: coordinates,
        ...
        if (currentZoom >= 14) {
          ...
          if (source && source._data) {
            ...
            const closestProperties = propertiesWithDistance.slice(
              0,
              clusterCount
            )

            console.log(
              `   Actual Properties in Cluster: ${closestProperties.length}`
            )
            console.log(
              `   Discrepancy: ${clusterCount !== closestProperties.length ? `⚠️ ${clusterCount - closestProperties.length} missing` : '✅ Match'}`
            )

            if (closestProperties.length > 0) {
              const realProperties: ListingResult[] = closestProperties.map(
                ({ feature }: any) => {
              ...
              if (mlsNumbers.length !== uniqueMlsNumbers.size) {
                console.log(
                  `   ⚠️ DUPLICATES IN CLUSTER TOOLTIP: ${mlsNumbers.length} total, ${uniqueMlsNumbers.size} unique`
                )

                // Log which ones are duplicated
                const counts = mlsNumbers.reduce(
                  (acc: Record<string, number>, mls: string) => {
                    acc[mls] = (acc[mls] || 0) + 1
                    return acc
                  },
                  {}
              ...
              const uniqueProperties = realProperties.filter(
                (property, index, self) =>
                  index ===
                  self.findIndex((p) => p.mlsNumber === property.mlsNumber)
              )
