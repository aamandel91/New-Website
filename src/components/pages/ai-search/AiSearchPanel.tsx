...
export function AiSearchPanel({
  ...
  const addLog = useCallback(
    (entry: { kind: AiActivityLogEntry['kind'] } & Record<string, any>) => {
      ...
      setLog((prev) => {
        ...
        return [
          ...filtered,
          { ...entry, id, ts: Date.now() } as AiActivityLogEntry
        ].slice(-50)
      })
    },
    []
  ...
  const runSearch = useCallback(
    async (raw: string) => {
      ...
      addLog({
        kind: 'status',
        text: FUN_STATUS_MESSAGES[
          Math.floor(Math.random() * FUN_STATUS_MESSAGES.length)
        ]
      })
              ...
              if (entry.kind === 'understood') {
                return (
                  <Box key={entry.id}>
                    <Typography
                      variant="body2"
                      sx={{ color: accentDark, fontWeight: 600 }}
                    >
                      {entry.summary}
                    </Typography>
                    <Stack
                    ...
                    >
                      {Object.entries(entry.filters).map(([k, v]) => (
                        <Chip
                          ...
                          size="small"
                          sx={{
                            bgcolor: tenant.visualIdentity.colors.primaryLight,
                            color: accentDark,
                            fontSize: 11
                          }}
                        />
