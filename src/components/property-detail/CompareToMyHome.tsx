...
const CompareToMyHome: React.FC<CompareToMyHomeProps> = ({
  ...
  const handleCompare = async () => {
    ...
    try {
      ...
      setError('Unable to generate an estimate for this address. Please check the details and try again.')
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleEmailSubmit = async () => {
    ...
    try {
      // Track with SureSend pixel
      ssIdentify({ email: email.trim() })

      // Also send to SureSend event API with tags
      await fetch('/api/suresend/event', {
        ...
        body: JSON.stringify({
          ...
          metadata: {
            ...
            comparedAddress: form.streetAddress,
            comparedCity: form.city,
            comparedZip: form.zipCode,
          },
        }),
      })
      ...
      // Silently fail — pixel tracking is best-effort
      setEmailSubmitted(true)
    } finally {
  ...
  const userPricePerSqft = result && userSqft > 0
    ? Math.round(result.estimatedValue / userSqft)
    : 0
            ...
            {!result && (
              <Box>
                <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
                  Enter your home details to see how it compares to this listing.
                </Typography>
              ...
              <Box>
                ...
                <Box
                  sx={{
                    mt: 2,
                    p: 2,
                    bgcolor: priceDifference > 0 ? 'success.light' : 'error.light',
                    borderRadius: 1,
                    textAlign: 'center',
                  }}
                >
                  ...
                  <Box sx={{ mt: 3 }}>
                    ...
                    <Box sx={{ display: 'flex', gap: 1, alignItems: 'flex-start' }}>
                      ...
                      <Button
                        ...
                        size="medium"
                      >
                        {emailLoading ? <CircularProgress size={20} color="inherit" /> : 'Submit'}
                      </Button>
                    </Box>
                  </Box>
                )}
                ...
                {emailSubmitted && (
                  <Box sx={{ mt: 3 }}>
                    ...
                    <Typography variant="body2" color="success.main" fontWeight={600}>
                      Thanks! We&apos;ll send you a detailed market analysis soon.
                    </Typography>
                  </Box>
                )}
              </Box>
