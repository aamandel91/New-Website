...
function getEventLabel(item: HistoryItemType): string {
  ...
  return listingLastStatusMapping[item.lastStatus as ListingLastStatus] || 'Listed'
}

function getEventColor(lastStatus: string): 'success' | 'warning' | 'info' | 'error' | 'default' {
  switch (lastStatus) {
    case 'Sld':
      return 'success'
    case 'New':
...
function getEventDate(item: HistoryItemType): string {
  ...
  if (item.timestamps?.listingEntryDate) return formatDate(item.timestamps.listingEntryDate)
  return '—'
}

function getEventPrice(item: HistoryItemType): string {
  if (item.soldPrice != null && item.soldPrice !== 0) return formatPrice(item.soldPrice)
...
const PropertyTransactionHistory: React.FC<PropertyTransactionHistoryProps> = ({
  ...
  const sorted = [...history].sort((a, b) => {
    const dateA = a.soldDate || a.listDate || a.timestamps?.listingEntryDate || ''
    const dateB = b.soldDate || b.listDate || b.timestamps?.listingEntryDate || ''
