...
export interface TrafficSourceData {
  ...
  referer?: string;
  landingPage?: string;
  trafficType?: 'ppc' | 'organic' | 'direct' | 'referral' | 'social' | 'email' | 'other';
}
 ...
 */
export function extractUtmParams(searchParams: URLSearchParams): TrafficSourceData {
  return {
 ...
 */
export function determineTrafficType(data: TrafficSourceData): TrafficSourceData['trafficType'] {
...
export function incrementPropertyViewCount(): number {
  ...
  try {
    const current = parseInt(sessionStorage.getItem(PROPERTY_VIEW_COUNT_KEY) || '0', 10);
